...
print("CAESAR TEXT READER")
print("Read all Caesar keys from 0 to 25")
...
def reader(text):
    ...
    for i in tqdm(range(12), desc='Comb', ascii=' >=', leave=False):
        ...
        for k in range(26):
            ...
            for char in replaced_text:
                ...
                if char.isalpha():
                    ...
                    if char.isupper():
                        result.append(chr((ord(char) + k - 65) % 26 + 65))
                    else:
                        result.append(chr((ord(char) + k - 97) % 26 + 97))
...
def main():
    ...
    while True:
        ...
        # Enter input text file to be processed
        while True:
            try:
                entry = input("Enter text file to process: ")
                if f"{entry.replace('.txt', '')}_caesar_saved.txt" in os.listdir('text'):
                    load = input("Detected save file, want to continue from save file (y/n)? ")
                    if load == 'n':
                        pass
                    else:
                        entry = f"{entry.replace('.txt', '')}_caesar_saved.txt"
        ...
        # Get file name
        entry = entry.replace('.txt', '').replace('_caesar_saved', '')
        ...
        for i in tqdm(range(len(sentences)), desc='Text', ascii=' >='):
            reader(sentences[i])
            save_file.pop(0)
            with open(f'text/{entry}_caesar_saved.txt', 'w') as f:
                f.write(' '.join(save_file))

        # Delete saved file once finished reading
        try:
            os.remove(f'text/{entry}_caesar_saved.txt')
        except OSError:
            pass
